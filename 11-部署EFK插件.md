...
---
>         image: 172.16.210.3:5000/gcr.io/google_containers/fluentd-elasticsearch:1.22
```
...
---
>         image: 172.16.210.3:5000/gcr.io/google_containers/kibana:v4.6.1-1
```
...
172.16.210.101   Ready     1d        v1.6.2

$ kubectl label nodes 172.16.210.101 beta.kubernetes.io/fluentd-ds-ready=true
node "172.16.210.101" labeled
```
...
``` bash
$ pwd
/root/follow-me-install-kubernetes-cluster/EFK
$ ls *.yaml
es-controller.yaml es-rbac.yaml es-service.yaml  fluentd-es-ds.yaml  kibana-controller.yaml  kibana-service.yaml fluentd-es-rbac.yaml
$ kubectl create -f .
$
...
kibana-logging-1917710937-4hgw5          1/1       Running   0          4m
...
``` bash
$ kubectl logs kibana-logging-1917710937-4hgw5 -n kube-system -f
...
1. 通过 kube-apiserver 访问：
    ...
    由于 kube-apiserver 开启了 RBAC 授权，浏览器访问 kube-apiserver 的时候使用的是证书，所以使用安全端口访问 kube-apiserver：

    浏览器访问 URL： `https://172.16.210.101:6443/api/v1/proxy/namespaces/kube-system/services/elasticsearch-logging`
